...
import pandas as pd
import logging
from process_data import get_data_cross_project,get_all_data
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("training_crossProject.log", mode='a'), 
...
threshold = 0.5
best_model_path = "best_cross_model.pt"
    ...
    for epoch in range(50):
        ...
        for step,batch in enumerate(tqdm(train_loader, desc="Training")):
            ...
            if contras_learn:
                ...
                loss = cls_loss + contrastive_loss*0.3
            else:
                loss = cls_loss
            loss.backward()
...
def tran_start(project_name,all_datas,hash_to_project):
    for _ in range(3):
        tran_dataset,valid_dataset,test_dataset= get_data_cross_project(project_name, all_datas,hash_to_project)
        ggnn_train(tran_dataset, tokenizer,val_dataset=valid_dataset)
        test_start(project_name=project_name,test_dataset=test_dataset)

    
    
def test_start(project_name,model=None,name='test',test_dataset=None):
    ...
    result_path = f"./test_prediction_results.csv"
    features_path=Path(CONSTANTS.repository_dir)/"features_all.pkl"
    ...
    cache_buggy_line = Path(CONSTANTS.repository_dir)/'cross_pj_line'/project_name/'changes_complete_buggy_line_level_cache.pkl'
    if os.path.exists(cache_buggy_line):
        commit2codes, idx2label = pickle.load(open(cache_buggy_line, 'rb'))
    else:
        buggy_line_filepath=Path(CONSTANTS.repository_dir)/'cross_pj_line'/project_name/'changes_complete_buggy_line_level.pkl'
...
def process_results(all_hashes, all_preds,all_probs=None,what="test",max_f1=0):
    ...
    real_probs=[]
    pred_label=[]
    with open(Path(CONSTANTS.repository_dir)/f"features_all.json", 'r',encoding='utf-8') as f:
        data = json.load(f)
        for i in range(n):
            hash=all_hashes[i]
            hashes.append(hash)
    ...
    if f1>max_f1:
        ...
        return f1
    return max_f1
def compute_contrastive_loss(embeddings, labels, temperature=1):
    """
...
if __name__ == "__main__":
    all_projects=['ant-ivy', 'commons-bcel', 'commons-beanutils', 
                  'commons-codec', 'commons-collections', 'commons-compress',
                  'commons-configuration', 'commons-digester', 'commons-lang',
                  'commons-math', 'commons-net', 'commons-scxml', 'commons-validator',
                  'commons-dbcp', 'commons-vfs', 'giraph', 'gora', 'opennlp', 'parquet-mr',
                  'commons-io', 'commons-jcs']
    repo_dir = CONSTANTS.repository_dir 
    all_datas,hash_to_project=get_all_data(Path(repo_dir)/"changes_all.pkl",tokenizer,device)
    for project in all_projects:
      with open("test@5_cross", 'a') as f:
        f.write(f"crossproject:{project}\n")
      tran_start(project,all_datas,hash_to_project)
